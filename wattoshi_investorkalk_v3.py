...
with col2:
    ...
    år = 1
    årlig_liste = [(0, round(beløp, 2), round(btc_kumulativ, 2))]
...
# Tillegg: demonstrasjonstabell med fast årlig startutbetaling og 20 % vekst i 10 år
st.markdown("---")
st.subheader("🔍 Eksempel: Passiv BTC-sparing med 20 % vekst fra 96 kr i 10 år")
...
st.write("|----|------------------|---------------------|")
st.write(f"| 0 | {beløp:.2f} | {akk:.2f} |")
for år in range(1, 10):
    beløp *= (1 + vekst)
    akk += beløp
    st.write(f"| {år} | {beløp:.2f} | {akk:.2f} |")
