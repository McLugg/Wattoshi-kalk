...
with col2:
    ...
    st.markdown("#### 💼 Fordeling av panelinntekten per rolle (før BTC-vekst)")
    ...
    st.subheader("📊 Kundeinntekt frem til nedbetalingstid")
    nedbet_år = int(nedbetaling_tid) + 1 if nedbetaling_tid else 0
    df_kort = pd.DataFrame({
        "År": list(range(nedbet_år)),
        "Årlig inntekt (NOK)": [round(årlig_kunde * ((1 + btc_growth) ** n), 4) for n in range(nedbet_år)],
        "Akkumulert inntekt (NOK)": [round(sum([årlig_kunde * ((1 + btc_growth) ** (n - i)) for i in range(n + 1)]), 4) for n in range(nedbet_år)]
    })
    st.dataframe(df_kort, use_container_width=True)
    ...
    st.subheader("📊 Kundeinntekt år 0–30")
    df = pd.DataFrame({
        "År": år,
